...
_ACTION_KEYWORDS = re.compile(
    r"напиш[иу]|написа[тьл]|отправ[ьи]|отправ(?:ить|ля[йл])|"
    r"пошли|пришли|скинь|скину[тьл]|"
    r"перешли|пересла[тьл]|переш[её]л|"
    r"закрепи|закрепить|найди|найти|наш[её]л|покажи|показа[тьл]|"
    r"прочитай|прочита[тьл]|"
    r"напомни|напомнить|поставь|создай|создать|удали|удалить|отмени|отменить|запланируй|"
    r"задач[аеуи]|напоминани[еяй]|дедлайн|таск|"
    r"send\s|write\s|forward|pin\s|search\s|remind|schedule|"
    r"посмотр[иеь]|проверь|провер[иья]|узна[йюь]|узнать|"
    r"спроси|спросить|запусти|выполни|сделай|сделать|"
    r"прислал|не\s+присла|ответил|написал",
    re.IGNORECASE,
)
...
_ACTION_INTENT_VERBS = re.compile(
    r"напиши|написать|отправь|отправить|пошли|скинь|перешли|переслать|"
    r"закрепи|закрепить|найди|найти|покажи|показать|прочитай|прочитать|"
    r"напомни|напомнить|поставь|создай|создать|удали|удалить|отмени|отменить|"
    r"запланируй|посмотри|посмотреть|проверь|проверить|узнай|узнать|спроси|спросить",
    re.IGNORECASE,
)
